      ...
      if (response.success) {
        ...
        if (pagesResponse.success) {
          ...
          const nodes = pages.map(page => ({
            ...
            url: page.url,
            type: 'page',
            domain: page.domain || new URL(page.url).hostname
          }));
...
try {
    ...
    // Create domain hulls (convex hulls around domain groups)
    Object.entries(domainGroups).forEach(([domain, groupNodes]) => {
    if (groupNodes.length >= 3) {
        // Only create hulls for domains with at least 3 nodes
        g.append('path')
        .attr('class', 'domain-hull')
        .attr('fill', d3.color('#6b8cae').copy({opacity: 0.1}))
        .attr('stroke', '#6b8cae')
        .attr('stroke-width', 1)
    ...
    // Simple function to create a convex hull path
    if (points.length < 3) return '';
    
    // Apply some padding to the hull
    const padding = 15;
    const paddedPoints = points.map(p => [p[0], p[1]]);
    
    // Create hull path (simplified)
