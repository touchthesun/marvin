...
def check_neo4j_running():
    ...
    try:
        ...
        # Wait for Neo4j to be ready with a more robust approach
        logger.info("Waiting for Neo4j to be ready...")
        max_attempts = 30  # Increase max attempts
        attempt = 0
        while attempt < max_attempts:
            attempt += 1
            logger.info(f"Checking Neo4j readiness (attempt {attempt}/{max_attempts})...")
            
            try:
                # Try to connect to Neo4j's HTTP interface to check if it's ready
                health_check = subprocess.run(
                    ["curl", "-s", "-I", "http://localhost:7474"],
                    capture_output=True,
                    text=True,
                    check=False
                )
                
                if health_check.returncode == 0 and "200 OK" in health_check.stdout:
                    logger.info("Neo4j is responding to HTTP requests")
                    # Additional wait to ensure bolt protocol is ready
                    time.sleep(5)
        ...
        logger.error("Neo4j failed to start within the expected time")
        return False
