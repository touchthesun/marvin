...
class TestKeywordBenchmark:
    @pytest.fixture(scope="class")
    def text_cleaner(self):
        return TextCleaner()
    
    @pytest.fixture(scope="class")
    def html_processor(self, text_cleaner):
        return HTMLProcessor(text_cleaner)
    
    @pytest.fixture(scope="class")
    def keyword_extractor(self, text_cleaner, html_processor):
    ...
    def test_benchmark_pages(self, keyword_extractor):
        """Run keyword extraction on all benchmark pages"""
        import time
        
        # Load benchmark pages
        benchmark_dir = Path(__file__).parent / 'pages'
        test_files = list(benchmark_dir.glob('**/*.html'))
        
        if not test_files:
            pytest.skip("No benchmark HTML files found")
        ...
        for file_path in test_files:
            ...
            # Get extracted text
            cleaned_text = self.clean_html_content(keyword_extractor, html_content)
            
            # Store results
            results[file_path.name] = BenchmarkResult(
                filename=file_path.name,
                file_path=file_path,
                keywords=[(kw.keyword, kw.score, kw.frequency) for kw in keywords],
                extracted_text=cleaned_text,
                processing_time=processing_time
            )
        
        # Run quality analysis
        metrics = self.analyze_keyword_quality(results)
        print(f"\n{metrics}")
        
        # Basic quality checks
        self.run_quality_checks(results)
        
        return results
    
    def analyze_keyword_quality(self, results: Dict[str, BenchmarkResult]) -> QualityMetrics:
    ...
    def run_quality_checks(self, results: Dict[str, BenchmarkResult]):
        """Run basic quality checks on the results"""
        for result in results.values():
            ...
            for keyword, score, freq in result.keywords:
                ...
                # Frequency should make sense
                assert freq > 0, f"Zero frequency for '{keyword}' in {result.filename}"
...
class TestKeywordExtraction:
    ...
    def create_extraction_fn(self, 
        ...
        def extraction_fn(text: str):
            ...
            print(f"Sample scores: {list(raw_keywords.items())[:5]}")
    ...
    def validate_results(self, results: List[ExtractionResult]):
        ...
        for result in results:
            ...
            assert len(result.keywords) > 0, f"No keywords found for {result.method}"
...
class TestRAKEExtraction(TestKeywordExtraction):
...
class TestTFIDFExtraction(TestKeywordExtraction):
...
class TestNERExtraction(TestKeywordExtraction):
...
class TestHybridExtraction(TestKeywordExtraction):
    ...
    def test_hybrid_extraction(self, keyword_extractor, benchmark_runner):
        test_files = benchmark_runner.load_test_files()
        hybrid_results = []
        
        extraction_fn = self.create_hybrid_extraction_fn(
            keyword_extractor=keyword_extractor
        )
        ...
        # Validate results
        self.validate_results(hybrid_results)
        self._validate_hybrid_results(hybrid_results)
